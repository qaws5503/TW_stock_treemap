...
def write_array(stock_data, category, category_percentage):
    ...
    for i in range(len(stock_data)):
        ...
        new = np.array([now["名稱"]+" "+str(now["漲跌  幅"])+" %", now["產業別"], int(now["市值  (億)"]), float(now["漲跌  幅"]*100)], dtype=object)
        array_data = np.append(array_data,new)
...
category = list(set(stock_data['產業別'].tolist()))

category_percentage = {}
for now_category in category:
    df = stock_data[stock_data['產業別']==now_category]
    category_percentage[now_category] = statistics.mean(df["漲跌  幅"])
